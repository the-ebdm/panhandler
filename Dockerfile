...
COPY --from=build --chown=bunjs:bunjs /app/packages/types/package.json ./packages/types/
COPY --from=build --chown=bunjs:bunjs /app/packages/core/dist ./packages/core/dist
COPY --from=build --chown=bunjs:bunjs /app/packages/core/package.json ./packages/core/
COPY --from=build --chown=bunjs:bunjs /app/packages/agents/dist ./packages/agents/dist
COPY --from=build --chown=bunjs:bunjs /app/packages/agents/package.json ./packages/agents/
